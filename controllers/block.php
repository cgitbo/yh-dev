...
class Block extends IController
{
	...
	public function order_delivery()
    {
		...
		$origin       = IFilter::act(IReq::get("origin"),'int');
	...
	public function doPay()
	{
		...
		if($order_id)
		{
			...
			if($orderRow['active_uid'])
			{
            	...
            	if ($isVip) {
					plugin::trigger('setCallback','/ucenter/order');
					$this->redirect('/site/success/message/'.urlencode("已经是vip会员了"));
					return;
				}
			}
		...
		{
			$sendData = $paymentInstance->getSendData(Payment::getPaymentInfo($payment_id,'order',$order_id,$origin));
		}
		else
		{
    ...
    */
	...
	function address_add()
	{
        ...
        $checkArray = $sqlData;
        unset($checkArray['telphone'],$checkArray['zip'],$checkArray['user_id'],$checkArray['is_default']);
