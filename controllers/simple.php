...
class Simple extends IController
{
	...
	function find_password_mobile()
	{
		...
		if($userRow)
		{
			...
			if($dataRow)
			{
				...
				{
					$this->redirect('/simple/restore_password/hash/'.$mobile_code.'/user_id/'.$userRow['user_id'].'/type/'.$type);
				}
			}
			else
			{
	...
	function do_restore_password()
	{
		...
		$type = IFilter::act(IReq::get("type")); // trans 为转账密码
		...
		$typeConf = ['default'=>'password', 'trans'=>'tran_password'];
		if(!$typeConf[$type]) IError::show(403,"非法请求");

		$tb_user->setData(array($typeConf[$type] => $pwd));
