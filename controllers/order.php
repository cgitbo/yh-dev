...
class Order extends IController implements adminAuthorization
{
	...
	public function shop_template()
	{
		...
		if($data['user_id'])
		{
			...
			$userRow = $userObj->getObj($data['user_id']);
	...
	public function order_report()
	{
		...
		$orderHandle = new IQuery('order as o');
		$orderHandle->order  = "o.id desc";
		$orderHandle->fields = "o.*,d.name as distribute_name,p.name as payment_name,u.username";
		$orderHandle->join   = $join;
		$orderHandle->where  = $where;
		$orderList = $orderHandle->find();

		$reportObj = new report('order');
		$reportObj->setTitle(array("订单编号","日期","用户名","配送方式","收货人","收货地址","电话","订单金额","支付方式","支付状态","发货状态","商品信息","商品数量","商品金额","订单备注"));
		...
		foreach($orderList as $k => $val)
		{
			...
			foreach($orderGoods as $good)
			{
				...
				$strGoods .= "<br />";
				$goodsNumber .= $good['goods_nums'].'<br />';
				$goodsPrice  .= $good['real_price'].'<br />';
			}
	...
	// 赠品操作
	public function gift_send_act()
	{
		$gift = IFilter::act(IReq::get('gift'));
		$gid  = IFilter::act(IReq::get('gid'));
		$oid  = IFilter::act(IReq::get('oid'));

		$query = new IModel('order');
		$result = $query->getObj('id = '. $oid. ' and gift is not null and pay_status = 1');

		$data = ['data' => 'SUCCESS'];

		if(!$result) $this->setError('订单不正确');

		$giftModel = new IModel('order_goods');
		$giftRes = $giftModel->getObj('id = '.$gid . ' and goods_id = ' . $gift.' and order_id = ' .$result['id']);
		
		if(!$giftRes) $this->setError('订单商品不正确');
		
		$res = $giftModel->setData(['is_send' => 3])->update('id = ' .$giftRes['id']);
		if(!$res) $this->setError('操作失败');

		if ($msg = $this->getError()) $data = $msg;

		$orderLog = new IModel('order_log');
		$orderLog->setData(array(
			'order_id' => $result['id'],
			'user'     => $this->admin['admin_name'],
			'action'   => '开单',
			'result'   => '成功',
			'note'     => '订单商品【'.$giftRes['goods_id'].'】由 '.$this->admin['admin_name'].' 开单成功',
			'addtime'  => ITime::getDateTime(),
		));
		$orderLog->add();
