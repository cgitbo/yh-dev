...
class Market extends IController implements adminAuthorization
{
	...
	function account_list()
	{
		...
		$accountObj = new IQuery('account_log l');
		$accountObj->join = 'left join user as u on u.id = l.user_id';
		$accountObj->fields = 'l.*, u.username';
		$accountObj->where = $where;
		$accountObj->order = 'l.id desc';
		$accountObj->page  = $page;
		$logList = $accountObj->find();

		$query = new IQuery('account_log l');
		$query->join = 'left join user as u on u.id = l.from_uid';
		$query->fields = 'l.id, u.username as from_user';
		$query->where = $where;
		$query->order = 'l.id desc';
		$query->page  = $page;
		$fromRow = $query->find();

		foreach ($fromRow as $key => $value) {
			if($value['id'] == $logList[$key]['id']) {
				$logList[$key]['from_user'] = $value['from_user'];
			}
		}

		$this->accountObj  = $accountObj;
		$this->accountList = $logList;
