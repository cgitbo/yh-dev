...
class IModel
{
	 ...
	 * 批量更新
	 *
	 * @param string $col       字段 : update $col = 1
	 * @param string $field     条件 : where  $field = 1
	 * @param array  $updateArr 数组 : $key 对应 $field $val对应$col
	 * @return void
	 */
	public function updateWithCase($col, $field, $updateArr)
	{
		$where = ' SET ' . $col . ' = CASE ' . $field;
		foreach ($updateArr as $key => $val)
		{
			...
			$where .= ' WHEN ' . $key . ' THEN ' . $val;
		}

		$keys = array_keys($updateArr);
		$str = implode(',', $keys);

		$where .= ' END WHERE ' . $field . ' in (' . $str . ')';
	...
	public function batchAdd()
	{
		...
		foreach($this->tableData as $key => $val)
		{
            ...
            //一维数组，插入一条
            if(count($this->tableData) == count($this->tableData,1)){
                $insertCol[] = '`'.$key.'`';
                $insertVal[] = '\''.$val.'\'';
                $addOneLeft  = '(';
                $addOneRight = ')';
		...
		//去重
        $insertCol = array_unique($insertCol);
        if(count($this->tableData) != count($this->tableData,1)){
            //批量添加定位
            foreach($this->tableData as $key => $val)
            {
                foreach ($insertCol as $k => $v)
                {
                    $insertVal[$key] .= '\''.$val[trim($v,'`')].'\',';
                }
            }
            //字符串处理
            foreach ($insertVal as $k => &$v)
            {
                $insertVal[$k] = trim($v,',');
                $insertVal[$k] = '('.$insertVal[$k].')';
            }
		}
		
		$sql = 'INSERT INTO '.$this->tableName.' ( '.join(',',$insertCol).' ) VALUES '.$addOneLeft.' '.join(',',$insertVal).' '.$addOneRight.' ';
		return $this->db->query($sql);
	}
}