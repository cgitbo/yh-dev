...
{
    public $actions = ["block" => ["wechat_push"]];
...
OEF;
	...
	public function reg()
	{
		...
		if(IClient::isWechat() == true)
		{
    		...
    		plugin::reg("onBeforeCreateAction@block@wxCallback",function(){
    			self::controller()->wxCallback = function(){$this->wxCallback();};
    		});
...
OEF;
	...
	public function getWxOpenId()
	{
		...
		//oauth回调处理
		if (!$code) throw new Exception("no code", 9001);

		if (!IValidate::check("^[\w\-]+$", $code)) throw new Exception("CODE码非法", 9001);

		$urlparam = array(
			'appid=' . wechat::$config['wechat_AppID'],
			'secret=' . wechat::$config['wechat_AppSecret'],
			'code=' . $code,
			'grant_type=authorization_code',
		);
		$apiUrl = "https://api.weixin.qq.com/sns/oauth2/access_token?" . join("&", $urlparam);
		$json   = file_get_contents($apiUrl, false, stream_context_create(wechat::$sslConfig));
		$result = JSON::decode($json);
		if (!isset($result['openid'])) {
			//code重复问题
			if (isset($result['errmsg']) && stripos($result['errmsg'], 'invalid code') !== false || stripos($result['errmsg'], 'code been used') !== false) {
				throw new Exception("code been used", 9001);
			}
			throw new Exception("根据code【" . $code . "】值未获取到open_id码:{$json}", 9001);
		}
		...
		if ($result && $state) {
			...
			$fun = isset(wechat::$bindConfig[$methodKey]) ? wechat::$bindConfig[$methodKey] : null;
			if ($fun && is_callable($fun)) {
				call_user_func($fun, $result, $param);
			}
			return $result;
		}
		throw new Exception("CODE码非法", 9001);
	}
