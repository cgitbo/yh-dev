 ...
 * @author nswe
 */
class app_wechat extends paymentPlugin
{
	//支付插件名称
    public $name = '微信APP支付';
	...
	public function getSendData($payment)
	{
		...
		if(is_array($resultArray))
		{
			...
			if(isset($resultArray['return_code']) && $resultArray['return_code'] == 'SUCCESS')
			{
				...
				return $this->getAppData($resultArray);
			}
	...
	// APP支付需要二次签名
	private function getAppData($sendData)
	{
		if(isset($sendData['prepay_id']) && $sendData['prepay_id'])
		{
			...
			$return['partnerid'] = $sendData['partnerid'];
		...
		return $sendData['err_code_des'] ? $sendData['err_code_des'] : '微信下单API接口失败';
	}
	...
	public function doPay($sendData)
	{
		if(isset($sendData['prepay_id']) && $sendData['prepay_id'])
		{
			...
			//签名结果与签名方式加入请求提交参数组中
			$orderDetailUrl       = IUrl::getHost().IUrl::creatUrl('/ucenter/order_detail/id/'.$sendData['order_id']);
			$return['sign']       = $mysign;
			$return['successUrl'] = urlencode(IUrl::getHost().IUrl::creatUrl('/site/success/message/'.urlencode('支付成功！')));
			$return['failUrl']    = urlencode($orderDetailUrl);//urlencode(IUrl::getHost().IUrl::creatUrl('/site/error/msg/'.urlencode('支付取消')));

			//发送APP的json参数
			$sendData = JSON::encode($return);
