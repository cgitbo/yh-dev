...
class Api
{
	...
	//curlRequest资源句柄
	private static $curlRequest = null;
	 ...
	 * @param Array  $headers  header数据
	 * @return String
	 */
	public static function httpRequest($postUrl, $postData = [], $headers = null)
	{
		if(self::$curlRequest == null)
		{
			self::$curlRequest = curl_init($postUrl);
			curl_setopt(self::$curlRequest, CURLOPT_POST, 1);
			curl_setopt(self::$curlRequest, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt(self::$curlRequest, CURLOPT_HEADER, false);
			curl_setopt(self::$curlRequest, CURLOPT_SSL_VERIFYPEER, 0);
			curl_setopt(self::$curlRequest, CURLOPT_SSL_VERIFYHOST, 0);

			if($headers)
			{
				curl_setopt(self::$curlRequest, CURLOPT_HTTPHEADER, $headers);
			}
		}

		curl_setopt(self::$curlRequest, CURLOPT_POSTFIELDS, http_build_query($postData));
		$result = curl_exec(self::$curlRequest);

		if(!$result)
		{
			$errorMsg = curl_error(self::$curlRequest);
			return array('status' => 'fail', 'error' => $errorMsg ? $errorMsg : "CURL异常出错");
		}
