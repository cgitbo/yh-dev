...
class Order_Class
{
	...
	public static function addGoodsCommentChange($order_id)
	{
		...
		// 是否远程商品
		$isYduiGoods = $orderRow['goods_type'] == 'Ydui';
		...
		foreach($orderList as $val)
		{
			...
			// 远程商品用远程接口
			if($isYduiGoods) {
				$issetGoods = Ydui::getData('getBothGoodsInfo', array('goods_id' => $val['goods_id']));
			} else {
	...
	public static function updateOrderStatus($orderNo,$admin_id = '',$note = '')
	{
		...
		else if($orderRow['pay_status'] == 0)
		{
			...
			{
				//减少库存量
				self::updateStore($orderGoodsListId,'reduce', $orderRow['goods_type']);
			}
			...
			// 如果是vip订单--废弃
			if($orderRow['vip_order_id'])
			{
				// 更新状态
				Team::vipOrderCallback($orderRow['id']);
			}
			...
			{
				// 更新状态
				Team::vipOrderServerCallback($orderRow['id']);
			}
	 ...
	 * @param string $origin 商品类型 暂时只处理Ydui商品
	 */
	public static function updateStore($orderGoodsId,$type = 'add', $origin = '')
	{
		...
		foreach($goodsList as $key => $val)
		{
			//商品所有数据在e兑那边操作,修改库存
			if ($origin == 'Ydui') {
				Ydui::api('BothUpdateStore', array(
	...
	{
		$data = array(0 => '未发',1 => '已发',2 => '已退',3 => '已领');
	...
	public static function getOrderGoods($order_id)
	{
		...
		foreach($orderGoodsList as $good)
		{
			...
			$temp['real_price'] = $good['real_price'];
			$goodList[] = $temp;
		}
		return $goodList;
	}
	...
	public static function getSearchCondition($search)
	{
		...
		if(isset($search['type']) && isset($search['content']) && $search['content'])
		{
			switch($search['type'])
			{
				...
				case "username":
				{
					$where[] = "u.username = '".$search['content']."'";
				}
				break;
	...
	public static function resetOrderProp($order_id)
	{
		...
		// Ydui 订单退款返优惠金额
		$curOrderList = $orderDB->getObj('id = '.$order_id. ' and goods_type = "Ydui"');
		if($curOrderList && $curOrderList['promotions'] > 0) {

			$userDB  = new IModel('user');
			$where   = 'id = '.$curOrderList['user_id'];
			$userRow = $userDB->getObj($where);

			$update  = $userRow['revisit'] + $curOrderList['promotions'];
			$res     = $userDB->setData(array('revisit' => $update))->update($where);
			...
			if($res) {
				$log = array(
					...
					'type'      => '0',
					'time'      => ITime::getDateTime(),
					'value'     => $curOrderList['promotions'],
					'value_log' => $update,
					'note'      => '用户'.$userRow['username'] . ' 操作订单退款，退回使用的优惠：'.$curOrderList['promotions'],
				);
	...
	public static function refund($refundId,$authorId,$type = 'admin',$way = 'balance')
	{
		...
		switch($way)
		{
			...
			case "balance":
			{
				...
				// Ydui 商品退款 允许退款金额为0，因为全部金额使用了vip消费去支付
				if($orderRow['goods_type'] == 'Ydui' && $amount == 0) $wayResult = true;
			}
			break;
		...
		{
			// 处理Ydui商品库存
			$origin = $orderRow['goods_type'];
			//库存增加
			self::updateStore($val['id'],'add', $origin);
	...
	{
		$orderDeTraceDB = new IModel('order_delivery_trace');
		$updateData     = array(
	...
	public static function isCancel($orderRow)
	{
	    ...
	    if(in_array($orderRow['status'],[1,2]) && !$isRefund && $orderRow['active_uid'] == '0')
	    {
	        return true;
	    }
	    return false;
