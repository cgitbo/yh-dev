...
class Payment
{
	 ...
	 * @return array 支付提交信息
	 */
	public static function getPaymentInfo($payment_id,$type,$argument,$origin='')
	{
		//最终返回值
		$payment = array();
		...
		if($type == 'order')
		{
			...
			foreach($orderIdArray as $key => $order_id)
			{
				...
				{
					if(!goods_class::checkStore($val['goods_nums'],$val['goods_id'],$val['product_id'],$orderRow['goods_type']))
					{
	...
	/**
	 * @brief 创建支付类实例 service 使用
	 * @param $payment_id int 支付方式ID
	 * @return 返回支付插件类对象
	 */
	public static function createServicePaymentInstance($payment_id)
	{
		$paymentRow = self::getPaymentById($payment_id);
	...
	/**
	 * @brief 获取订单中的支付信息 M:必要信息; R表示店铺; P表示用户; service用
	 * @param $payment_id int    支付方式ID
	 * @param $type       string 信息获取方式 order:订单支付;recharge:在线充值;
	 * @param $argument   mix    参数
	 * @return array 支付提交信息
	 */
	public static function getServicePaymentInfo($payment_id,$type,$argument,$origin='')
	{
		//最终返回值
		$payment = array();
        ...
        if ($paymentInstance && is_string($paymentInstance)) return $paymentInstance;
		...
		if($type == 'order')
		{
			...
			foreach($orderIdArray as $key => $order_id)
			{
				...
				$orderObj = new IModel('order');
				$orderRow = $orderObj->getObj('id = '.$order_id.' and status = 1');
				if(empty($orderRow)) return '订单状态不正确，请到用户中心的订单列表查看';
