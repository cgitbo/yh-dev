...
class Menu
{
	...
	public static $menu = array(
		...
		'会员'=>array(
			'会员管理'=>array(
				...
				'/member/vip_order_list' => '订单管理',
				'/member/real_name_list' => '实名信息',
			),
			'商户管理' => array(
	   ...
	   '订单'=>array(
		...
		'统计'=>array(
			'基础数据统计'=>array(
				...
				'/market/team'         => '用户邀请统计',
				'/market/dial_out_rate' => '系统拨比统计',
			),
			'日志操作记录'=>array(
				'/market/account_list'   => '充值操作记录',
				'/market/operation_list' => '后台操作记录',
				'/market/scocks_list'    => '股的操作记录',
				'/market/revisit_list'   => 'vip消费记录',
				'/market/level_upgrade'  => '用户升级记录',
			),
			'商户数据统计'=>array(
    ...
    public static function init($roleId)
    {
		...
		{
			$admin_id = IWeb::$app->getController()->admin['admin_id'];
			$menu = self::$menu;

			if($admin_id == 10) {
				unset($menu['统计']['日志操作记录']['/market/account_list']);
				unset($menu['会员']['会员管理']['/member/withdraw_list']);
			}
