...
class Cart
{
	...
	*/
	 ...
	 * 构造函数
	 *
	 * @param string $origin 购物车商品来源 存的是ydui的商品
	 */
	...
	private function getGoodInfo($gid, $type = 'goods')
	{
		...
		if($this->origin == 'Ydui')
		{
			if($type == 'goods') return $dataArray = Ydui::getData('getBothGoodsInfo', array('goods_id' => $gid));
			return $dataArray = Ydui::getData('getBothProductInfo', array('id' => $gid));
		}
		//商品方式
		if($type == 'goods')
		{
	...
	public function cartFormat($cartValue)
	{
		...
		// ydui商品单独计算
		if($this->origin == 'Ydui') {
			$result = Api::run('getYduiCartFormat', $cartValue, $result);
			return $result;
		}
