...
class statistics
{
	 ...
	 * @param string $start 开始日期 Y-m-d
	 * @param string $end   结束日期 Y-m-d
	 * @return array array(日期时间 => vip用户的人数);
	 */
	public static function userVipReg($start = '',$end = '')
	{
		$db = new IQuery('user');
		$db->fields = 'count(*) as yValue,`check_time`';
		$db->where  = 'is_empty = 0 and level >= 11';
		return self::ParseCondition($db,'check_time',$start,$end);
	}
}