...
class Delivery
{
	 ...
	 *	)
	 */
	public static function getDelivery($province,$delivery_id,$goods_id,$product_id = 0,$num = 1,$origin = '')
	{
		//获取默认的配送方式信息
		...
		foreach($goods_id as $key => $gid)
		{
			...
			if($pid > 0)
			{
				if($origin == 'Ydui') {
					// Ydui 商品处理
					$goodsRow = Ydui::getData("getBothProductInfo",array('id'=>$pid));
					$goodsRow['seller_id'] = 0;
				}
				else {
			...
			else
			{
				if($origin == 'Ydui') {
					// Ydui 商品处理
					$goodsRow = Ydui::getData("getBothGoodsInfo",array('goods_id'=>$gid));
					$goodsRow['seller_id'] = 0;
				}
				else {
		...
		foreach($sellerGoods as $seller_id => $data)
		{
			...
			// Ydui 商品处理
			if($origin == 'Ydui') $deliveryRow = $deliveryDefaultRow;
